...
-- Crea la base de datos principal SIGHC

IF DB_ID('SIGHC') IS NULL
BEGIN
...
CREATE TABLE Consultas (
    ...
    Talla DECIMAL(5,2),
    IMC AS (
        CASE WHEN Talla > 0 THEN Peso / (Talla * Talla) ELSE NULL END
    ) PERSISTED,
    SaturacionO2 INT,
    ...
    CONSTRAINT FK_Consultas_Citas FOREIGN KEY (IdCita) REFERENCES Citas(IdCita),
...
CREATE TABLE Diagnosticos (
    ...
    FechaRegistro DATETIME2 DEFAULT SYSDATETIME() NOT NULL,
    CONSTRAINT FK_Diagnosticos_Consultas FOREIGN KEY (IdConsulta) REFERENCES Consultas(IdConsulta),
    CONSTRAINT FK_Diagnosticos_CIE10 FOREIGN KEY (CodigoCIE10) REFERENCES CIE10(CodigoCIE10)
);
GO

-- TABLA 08: Medicamentos
...
CREATE TABLE Tratamientos (
    ...
    CONSTRAINT FK_Tratamientos_Diagnosticos FOREIGN KEY (IdDiagnostico) REFERENCES Diagnosticos(IdDiagnostico),
    CONSTRAINT FK_Tratamientos_Medicamentos FOREIGN KEY (IdMedicamento) REFERENCES Medicamentos(IdMedicamento)
);
GO
...
-- Pacientes
CREATE NONCLUSTERED INDEX IX_Pacientes_DNI
ON Pacientes(DNI);

CREATE NONCLUSTERED INDEX IX_Pacientes_Nombres
ON Pacientes(Nombres, Apellidos);

CREATE NONCLUSTERED INDEX IX_Pacientes_Estado
ON Pacientes(Estado)
WHERE Estado = 'A';
GO
...
(
    SELECT
        ...
        d.TipoDiagnostico,
        STRING_AGG(CONCAT(med.NombreGenerico, ' ', t.Dosis, ' ', t.Frecuencia), '; ')
            AS Tratamientos
    FROM Consultas c
    ...
    GROUP BY
        c.FechaConsulta, m.Nombres, m.Apellidos,
        e.NombreEspecialidad, c.MotivoConsulta,
        d.CodigoCIE10, d.DescripcionDiagnostico, d.TipoDiagnostico
);
GO
...
BEGIN
    ...
    BEGIN
        ...
        SELECT
            ...
            @UsuarioID,
            @UsuarioNombre,
            (SELECT * FROM inserted x WHERE x.IdPaciente = i.IdPaciente FOR JSON PATH)
        FROM inserted i;
    END
    ...
    IF @Operacion = 'UPDATE'
    BEGIN
        INSERT INTO AuditLog (TablaAfectada, Operacion, IdRegistro, UsuarioID, UsuarioNombre,
    ...
    BEGIN
        ...
        SELECT
            ...
            @UsuarioID,
            @UsuarioNombre,
            (SELECT * FROM deleted x WHERE x.IdPaciente = d.IdPaciente FOR JSON PATH)
        FROM deleted d;
    END
END;
GO
...
BEGIN
    ...
    BEGIN
        ...
        SELECT
            ...
            (SELECT * FROM inserted x WHERE x.IdDiagnostico = i.IdDiagnostico FOR JSON PATH)
        FROM inserted i;
    END
    ...
    BEGIN
        INSERT INTO AuditLog (TablaAfectada, Operacion, IdRegistro, UsuarioID, UsuarioNombre, ValoresAnteriores)
        SELECT
            'Diagnosticos', @Operacion, d.IdDiagnostico, @UsuarioID, @UsuarioNombre,
            (SELECT * FROM deleted x WHERE x.IdDiagnostico = d.IdDiagnostico FOR JSON PATH)
        FROM deleted d;
    END
END;
GO
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT @Correlativo =
            ISNULL(MAX(CAST(RIGHT(NroHistoriaClinica, 5) AS INT)), 0) + 1
        FROM Pacientes
        WHERE NroHistoriaClinica LIKE 'HC-' + @Anio + '-%';

        SET @NroHistoriaOut = 'HC-' + @Anio + '-' +
            RIGHT('00000' + CAST(@Correlativo AS VARCHAR), 5);
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT @Correlativo =
            ISNULL(MAX(CAST(RIGHT(CodigoCita, 6) AS INT)), 0) + 1
        FROM Citas
        ...
        VALUES (
            @CodigoCitaOut, @IdPaciente, @IdMedico, @FechaCita, @HoraInicio,
            @HoraFin, @MotivoConsulta, @TipoCita, @UsuarioRegistro
        );
...
BEGIN
    ...
    BEGIN TRY
        ...
        INSERT INTO Consultas (
            ...
            Peso, Talla, MotivoConsulta, ExamenFisico
        )
        VALUES (
            @IdCita, @IdPaciente, @IdMedico,
            @PresionArterial, @Temperatura, @FrecuenciaCardiaca,
...
-- Permisos Farmacia (en tu script original estaba etiquetado como Enfermera por error)
...
-------------------------------------------------------
-- JOB: Backup FULL Semanal
-------------------------------------------------------
EXEC sp_add_job 
    @job_name = N'SIGHC_Backup_Full_Semanal';
GO
...
EXEC sp_add_jobstep
    ...
    @command = N'
        ...
        SET @FileName = ''C:\Backups\SIGHC\SIGHC_FULL_'' +
...
EXEC sp_add_schedule 
    @schedule_name = N'SIGHC_Domingo_2AM',
    @freq_type = 8,              -- semanal
...
EXEC sp_add_jobstep
    @job_name = N'SIGHC_Backup_Diferencial_Diario',
    @step_name = N'Realizar Backup DIFF',
    @subsystem = N'TSQL',
    @command = N'
        DECLARE @FileName NVARCHAR(500);
        SET @FileName = ''C:\Backups\SIGHC\SIGHC_DIFF_'' +
                        CONVERT(VARCHAR, GETDATE(), 112) + ''_'' +
                        REPLACE(CONVERT(VARCHAR, GETDATE(),108),'':'','''') +
                        ''.bak'';
...
EXEC sp_add_schedule 
    @schedule_name = N'SIGHC_Diario_2AM',
    @freq_type = 4,              -- diario
...
EXEC sp_add_job 
    @job_name = N'SIGHC_Backup_Log_15min';
GO

EXEC sp_add_jobstep
    @job_name = N'SIGHC_Backup_Log_15min',
    @step_name = N'Realizar Backup LOG',
    @subsystem = N'TSQL',
    @command = N'
        DECLARE @FileName NVARCHAR(500);
        SET @FileName = ''C:\Backups\SIGHC\Logs\SIGHC_LOG_'' +
                        CONVERT(VARCHAR, GETDATE(), 112) + ''_'' +
                        REPLACE(CONVERT(VARCHAR, GETDATE(),108),'':'','''') +
                        ''.trn'';
...
EXEC sp_add_schedule 
    @schedule_name = N'SIGHC_Cada_15_Minutos',
    @freq_type = 4,                 -- diario
    @freq_interval = 1,
    @freq_subday_type = 4,          -- cada X minutos
    @freq_subday_interval = 15,     -- 15 min
...
-- 11_simulacion_desastre.sql
-- Simulación de pérdida de datos + backup de log previo

USE SIGHC;
GO

-- Contexto de auditoría (simula usuario autenticado)
EXEC sys.sp_set_session_context 'UsuarioID', 1;
EXEC sys.sp_set_session_context 'UsuarioNombre', 'AdminPrueba';
GO

-- Insertar pacientes de prueba que luego se "pierden"
INSERT INTO Pacientes (
    NroHistoriaClinica, Nombres, Apellidos, DNI,
    FechaNacimiento, Sexo, GrupoSanguineo, Direccion,
    Telefono, Email, AntecedentesFamiliares,
    AntecedentesPersonales, Alergias, UsuarioRegistro
)
VALUES
('HC-TEST-00002','Maria','Lopez','87654321','1995-03-12','F','A+','Calle Lima 321','900111222','maria@test.com','Ninguno','Ninguno','Ninguna',1),
('HC-TEST-00003','Pedro','Gomez','11223344','1988-11-02','M','O-','Av. Grau 455','955667788','pedro@test.com','Asma','Ninguno','Polen',1);
GO

-- Backup del log ANTES del desastre
BACKUP LOG SIGHC
TO DISK = 'C:\Backups\SIGHC\Logs\PRE_DESASTRE.trn'
WITH INIT, CHECKSUM;
GO

-- Simular desastre: borrado de datos
DELETE FROM Pacientes
WHERE DNI IN ('87654321','11223344');
GO
...
-- 12_restauracion_completa.sql
-- Restaurar la base al estado justo antes del "desastre"

USE master;
GO

-- Forzar modo SINGLE_USER para restaurar
ALTER DATABASE SIGHC SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
GO

-- Restaurar FULL (sin recovery, para poder aplicar log)
RESTORE DATABASE SIGHC
FROM DISK = 'C:\Backups\SIGHC\SIGHC_FULL.bak'
WITH NORECOVERY, REPLACE;
GO

-- Aplicar log PRE_DESASTRE
RESTORE LOG SIGHC
FROM DISK = 'C:\Backups\SIGHC\Logs\PRE_DESASTRE.trn'
WITH RECOVERY;
GO

-- Volver a multiusuario
ALTER DATABASE SIGHC SET MULTI_USER;
GO

-- Verificar que los pacientes regresaron
USE SIGHC;
GO
